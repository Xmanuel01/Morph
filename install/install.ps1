...
$installDir = Join-Path $env:USERPROFILE ".morph\\bin"
...
Copy-Item -Path (Join-Path $extract "morph.exe") -Destination (Join-Path $installDir "morph.exe") -Force
if (Test-Path (Join-Path $extract "morph_native.dll")) {
    Copy-Item -Path (Join-Path $extract "morph_native.dll") -Destination $installDir -Force
}
...
& (Join-Path $installDir "morph.exe") --version
Write-Host "Installed Morph to $installDir"
