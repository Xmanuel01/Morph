...
This repository contains the v3.5.0 development line in Rust.
...
- the shipped `examples/` audit remains the package-aware proof for validation
  examples; the audited-surface bundle is the smaller executable set that must
  also pass `selfhost-ci`/`replace-check`/`mainline-ci` in one materialized run
...
- Compatibility/deprecation governance and self-host fallback workflow docs for v3.0.0 release readiness
- Version-neutral release pipeline, deterministic packaging, checksum verification, SBOM generation, RC evidence-archive gates for v3.0.0 sign-off
...
- Final QA findings for the quality-recovery program:
...
- Strict-contract enforcement in v3.0.0:
