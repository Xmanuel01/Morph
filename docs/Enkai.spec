# Enkai Language Specification (v1.0 draft)

Status: draft. This document defines the surface syntax and core runtime rules
for Enkai v1.0. The language is block-structured using "::" and avoids curly
braces for blocks.
...
- enkai.lock
- src/main.enk (entry)
- src/lib.enk (library entry)
- src/<path>.enk for modules

Module resolution:
- use foo.bar resolves to src/foo/bar.enk or src/foo/bar/index.enk.
