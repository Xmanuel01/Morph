...
struct Symbols {
    ...
    forward_l2: unsafe extern "C" fn(*const c_char) -> i64,
    backward: unsafe extern "C" fn(i64) -> c_int,
    item: unsafe extern "C" fn(i64) -> f64,
    dist_init: unsafe extern "C" fn(i32, i32) -> c_int,
    dist_allreduce: unsafe extern "C" fn(*const c_char) -> c_int,
    dist_shutdown: unsafe extern "C" fn() -> c_int,
...
impl Backend {
    ...
    pub fn train_step_on_device(&mut self, _device_idx: usize) -> Result<f32, RuntimeError> {
        ...
        let loss_handle = unsafe { (self.symbols.forward_l2)(c.as_ptr()) };
        if loss_handle == 0 {
            return Err(last_error());
        }
        // Backward
        ...
        // Read loss scalar
        let loss = unsafe { (self.symbols.item)(loss_handle) } as f32;
        Ok(loss)
    }

    pub fn eval_step_on_device(&mut self, device_idx: usize) -> Result<f32, RuntimeError> {
        self.train_step_on_device(device_idx)
    }
    ...
    pub fn dist_init(&self, world_size: i32, rank: i32) -> Result<(), RuntimeError> {
        let rc = unsafe { (self.symbols.dist_init)(world_size, rank) };
        if rc == 0 {
            Ok(())
        } else {
            Err(last_error())
    ...
    pub fn dist_allreduce_grads(&self, grads: &[i64]) -> Result<(), RuntimeError> {
        ...
        let c = CString::new(json).unwrap();
        let rc = unsafe { (self.symbols.dist_allreduce)(c.as_ptr()) };
        if rc == 0 {
            Ok(())
        } else {
            Err(last_error())
    ...
    pub fn dist_shutdown(&self) {
        unsafe {
            let _ = (self.symbols.dist_shutdown)();
        }
    }
...
unsafe fn load_symbols(lib: &Library) -> Result<Symbols, RuntimeError> {
    ...
    Ok(Symbols {
        ...
        dist_init: sym!(
            b"enkai_dist_init\0",
            unsafe extern "C" fn(i32, i32) -> c_int
        ),
        dist_allreduce: sym!(
            b"enkai_dist_allreduce_sum_multi\0",
            unsafe extern "C" fn(*const c_char) -> c_int
        ),
        dist_shutdown: sym!(b"enkai_dist_shutdown\0", unsafe extern "C" fn() -> c_int),
        ckpt_save: sym!(
...
fn last_error() -> RuntimeError {
    RuntimeError::new("native backend error (see enkai_tensor_last_error)")
}
