...
def main() -> int:
    ...
    p.add_argument('--contract', default='enkai/contracts/v3_5_0_release_line_start_baseline.json')
    p.add_argument('--output', default='artifacts/readiness/v3_5_0_release_line_start_baseline.json')
    args=p.parse_args()
    ...
    for rel in ['enkai/Cargo.toml','enkaic/Cargo.toml','enkairt/Cargo.toml','enkai_native/Cargo.toml','enkai_tensor/Cargo.toml','enkai.toml']:
        ...
        if '3.5.0' not in text:
            failures.append(f'{rel} does not contain version 3.5.0')
