...
def main() -> int:
    ...
    p.add_argument('--contract', default='enkai/contracts/v3_5_0_objective_set_freeze.json')
    p.add_argument('--output', default='artifacts/readiness/v3_5_0_objective_set_freeze.json')
    args=p.parse_args()
    ...
    for rel in contract.get('required_files', []):
        ...
        if not path.is_file():
            ...
            continue
        if rel.endswith('.json') and load_json(path).get('all_passed') is not True:
