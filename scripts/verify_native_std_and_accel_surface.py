...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify the strict-selfhost native std/accel surface."
    )
    parser.add_argument(
        "--contract",
        default="enkai/contracts/selfhost_native_std_and_accel_surface_v3_3_0.json",
        help="Path to the native std/accel surface contract JSON",
    )
    parser.add_argument(
        "--output",
        default="artifacts/readiness/strict_selfhost_native_std_and_accel_surface.json",
    ...
    for part in dotted_path.split("."):
        if isinstance(current, list):
            try:
                ...
                raise KeyError(dotted_path) from err
...
def main() -> int:
    ...
    report = {
        ...
        "failures": failures,
        "reports": {
            report_rel: report_cache.get(report_rel)
            for report_rel in contract["required_reports"]
        },
    }
