...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify the first explicit v3.4.0 release-line normalization scope."
    )
    parser.add_argument(
        "--contract",
        default="enkai/contracts/v3_4_0_release_line_baseline.json",
        help="Path to the v3.4.0 baseline contract JSON",
    )
    parser.add_argument(
        "--output",
        default="artifacts/readiness/v3_4_0_release_line_baseline.json",
...
def main() -> int:
    ...
    for composite_key, expected in contract.get("required_json_fields", {}).items():
        ...
        report = json_cache.get(rel_path)
        if report is None:
            path = (repo_root / rel_path).resolve()
            if not path.is_file():
