...
";
    ...
    for item in module.items {
        match item {
            enkaic::ast::Item::Fn(decl) if decl.name == "pub_fn" => {
                assert!(decl.is_pub);
                saw_pub_fn = true;
            }
            enkaic::ast::Item::Fn(decl) if decl.name == "priv_fn" => {
                assert!(!decl.is_pub);
                saw_priv_fn = true;
            }
            enkaic::ast::Item::Type(decl) if decl.name == "PubType" => {
                assert!(decl.is_pub);
                saw_pub_type = true;
            }
            enkaic::ast::Item::Type(decl) if decl.name == "PrivType" => {
                assert!(!decl.is_pub);
                saw_priv_type = true;
            }
            enkaic::ast::Item::Enum(decl) if decl.name == "PubEnum" => {
                assert!(decl.is_pub);
                saw_pub_enum = true;
            }
            enkaic::ast::Item::Enum(decl) if decl.name == "PrivEnum" => {
                assert!(!decl.is_pub);
                saw_priv_enum = true;
            }
            enkaic::ast::Item::Use(decl)
