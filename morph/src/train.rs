...
use morphc::bytecode::{ByteFunction, Chunk, Constant, Instruction, Program};
use morphc::compiler::{compile_package, CompileError};
use morphc::modules::load_package;
use morphc::{TypeChecker, TypeError};
use morphrt::checkpoint::{
    latest_checkpoint, load_checkpoint, rotate_checkpoints, save_checkpoint, CheckpointMeta,
    CheckpointState,
};
use morphrt::dataset::{resolve_dataset_paths, Batch, DatasetConfig, DatasetStream};
use morphrt::object::Obj;
use morphrt::tokenizer::{Tokenizer, TrainConfig as TokenizerTrainConfig};
...
mod tests {
    ...
    fn escape_morph_string(input: &str) -> String {
        input.replace('\\', "\\\\").replace('\"', "\\\"")
    }

    fn write_config(path: &Path, json: &serde_json::Value) -> Result<(), String> {
        let text = json.to_string();
        let escaped = escape_morph_string(&text);
    ...
    fn train_runs_and_resumes() {
        ...
        let config_path = dir.path().join("config.morph");
